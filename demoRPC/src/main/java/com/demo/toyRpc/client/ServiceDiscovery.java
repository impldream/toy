...
public class ServiceDiscovery {
    ...
    private ZooKeeper connectServer() {
        ZooKeeper zk = null;
        try {
            zk = new ZooKeeper(registryAddress, Config.ZK_SESSION_TIMEOUT, new Watcher() {
                @Override
    ...
    private void watchNode(final ZooKeeper zk) {
        try {
            //获取子节点列表
            List<String> nodeList = zk.getChildren(Config.ZK_REGISTRY_PATH, new Watcher() {
                @Override
                public void process(WatchedEvent event) {
            ...
            for (String node : nodeList) {
                byte[] bytes = zk.getData(Config.ZK_REGISTRY_PATH + "/" + node, false, null);
